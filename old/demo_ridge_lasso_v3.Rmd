...
outliers <- boxplot(train$median_income, plot = FALSE)$out
train[train$median_income %in% outliers, "median_income"] = NA
train<- kNN(train,variable=c("median_income"),k=7)
train<- subset(train, select = -c(median_income_imp) )
...
outliers <- boxplot(test$median_income, plot = FALSE)$out
test[test$median_income %in% outliers, "median_income"] = NA
test<- kNN(test,variable=c("median_income"),k=7)
test<- subset(test, select = -c(median_income_imp) )

```

```{r}
# Transformacion variables data entrenamiento
# ===============================================

train$longitude <- RE.Johnson(train$longitude)$transformed
train$latitude <- RE.Johnson(train$latitude)$transformed
train$housing_median_age <- asin((train$housing_median_age/max(train$housing_median_age))^(1/2))
train$total_rooms <- RE.Johnson(train$total_rooms)$transformed
train$total_bedrooms <- RE.Johnson(train$total_bedrooms)$transformed
train$population <- RE.Johnson(train$population)$transformed
train$households <- RE.Johnson(train$households)$transformed
train$median_income <- RE.Johnson(train$median_income)$transformed
train$median_house_value <- RE.Johnson(train$median_house_value)$transformed

# Transformacion variables data prueba
# ===============================================
test$longitude <- RE.Johnson(test$longitude)$transformed
test$latitude <- RE.Johnson(test$latitude)$transformed
test$housing_median_age <- asin((test$housing_median_age/max(test$housing_median_age))^(1/2))
test$total_rooms <- RE.Johnson(test$total_rooms)$transformed
test$total_bedrooms <- RE.Johnson(test$total_bedrooms)$transformed
test$population <- RE.Johnson(test$population)$transformed
test$households <- RE.Johnson(test$households)$transformed
test$median_income <- RE.Johnson(test$median_income)$transformed
...
# Reemplazo de outlier por el metodo knn de los datos de entrenamiento
# ==============================================================================
outliers <- boxplot(train$longitude, plot = FALSE)$out
train[train$longitude %in% outliers, "longitude"] = NA
train<- kNN(train,variable=c("longitude"),k=7)
train<- subset(train, select = -c(longitude_imp) )
...
```{r echo=TRUE}
chart.Correlation(dataset[, c(1:9)],hist=T, cex=2)
```
```{r echo=TRUE}
set.seed(222)
index<-createDataPartition(dataset$id, p=0.7, list=FALSE)
